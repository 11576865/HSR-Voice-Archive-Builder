...
- Quick Mode now understands mixed-language packages: when duplicate WAV basenames come from different top-level folders, the folders are classified by name keywords or sampled LAB text, the folder matching the selected source text language becomes the primary audio stream, and the other folders contribute same-stem LAB text only as official target subtitles (their audio never enters the continuous FLAC, mirroring the official-Chinese-package flow). Cross-folder duplicate basenames therefore resolve to one indexed voice plus one official-text match instead of blocking the scan; duplicate basenames inside one folder still block, and folders whose language cannot be identified produce a blocker explaining how to rename them. The dashboard scan report shows the detected folder roles, and the project materializes the split under `.generated/language_folders/` so builds never see the mixed archive.
