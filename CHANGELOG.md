...
- GPT-SoVITS export validates source-text and declared audio languages, checks source WAV hashes against the completed archive, and writes a per-sample provenance CSV. CI now runs the pytest exporter cases alongside unittest.
- The dashboard distinguishes a completed build with unavailable WAVs or missing target text, and the Pages version/description matches the local application.

- Same-basename WAV files in different package directories (for example the Trailblazer's parallel male/female chapter trees) no longer block Quick Mode. Every package member now carries a stable `source_member_id` (its package-relative path) through scan, generated index, manifest, corrected CSV and subtitle overrides, so identically named files keep distinct identities end to end. When an index row matches a duplicated basename, the row is bound to exactly one member by its indexed SHA-256 or by identical same-stem LAB text; members that cannot be bound reliably are demoted to the appendix (keeping their audio) instead of failing the scan, and the preflight reports "发现 N 组同名，已自动消歧 n 条 / 待确认 m 条".
- Packages whose every WAV has same-stem LAB text no longer require an external index for any source language (previously only non-EN sources with a built-in remote index qualified): when no local/remote index covers the package, or the best coverage falls below the 50% floor, Quick Mode orders the archive by the package's own directory structure and takes source text from the LAB files. This unblocks the EN Trailblazer package, whose filenames the remote workbook does not list.
- Incremental remote updates now preserve package directory structure when merging newly downloaded audio into the combined source, so structured packages no longer have same-basename members overwrite each other in one flat folder; appended index rows record their `source_member_id`.
- Subtitle-review overrides (`final_chs`) record the entry's `source_member_id` and use it when re-applying edits, so a same-basename sibling never receives or loses another member's override; bare-filename fallback matching is suppressed for filenames that are ambiguous within the project.
- The Subtitle Review editor now tracks the active subtitle by id (stable across list refreshes and filtering), highlights search-keyword matches in the list, and saves the previously edited entry in the background when you navigate instead of requiring a "保存并下一条" button; the progress card also exposes canonical ids (`progressTitle`/`progressDesc`/`progressMeta`/`progressLog`) for the launch/server progress templates, and job polling retries transient fetch failures inline (up to 5 attempts with linear backoff) before falling back to the outer backoff loop.
...
- Quick Mode now allows partial index coverage: when the best local or remote index covers only part of the package WAV names, the scan still builds, and uncovered or conflicting files keep their audio in an appendix section appended at the end without indexed order or subtitles. Scans still block below a 50% coverage floor or when no index row matches any WAV name. Package WAV names that collide with conflicting duplicate remote-index rows are treated as uncovered instead of failing the scan, and dominant-character share below 75% is now a warning rather than a scan blocker.
- Relaxed empty source text through the builder: index rows without source text keep their audio and render without subtitles, while the translation queue skips them instead of failing.
